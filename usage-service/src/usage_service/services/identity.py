    ...
    def verify(self, token: str) -> Identity:
        ...
        if not key and self.settings.jwt_public_key_path:
            try:
                ...
                raise UsageError(503, "AUTH_CONFIGURATION_ERROR", "JWT verification is unavailable") from exc
        try:
            payload = cast(
                dict[str, Any],
                jwt.decode(
            ...
            if payload["token_type"] != "access":  # noqa: S105
                raise UsageError(401, "INVALID_TOKEN", "Access token is invalid")
            return Identity(
            ...
            raise UsageError(401, "TOKEN_EXPIRED", "Access token has expired") from exc
        except (jwt.PyJWTError, KeyError, ValueError) as exc:
            raise UsageError(401, "INVALID_TOKEN", "Access token is invalid") from exc
