...
from content_service.models import CONTENT_SCHEMA, Base
...
async def online() -> None:
    ...
    async with engine.connect() as connection:
        await connection.execute(text(f'CREATE SCHEMA IF NOT EXISTS "{CONTENT_SCHEMA}"'))
...
if context.is_offline_mode():
    configure()
    context.execute(f'CREATE SCHEMA IF NOT EXISTS "{CONTENT_SCHEMA}"')
    with context.begin_transaction():
        context.run_migrations()
else:
    asyncio.run(online())
