...
export const productViews: Record<string, ProductView> = {
  ...
  scheduler: {
    ...
    eyebrow: "Calendar",
    description: "Attach content to future publish times and hand due items to the publishing pipeline.",
...
export function sessionFor(role: Role, name: string, email: string): AuthSession {
  return {
    user: {
      ...
      name,
      email,
      accountId: role === "SuperAdmin" ? "platform" : "tenant_orion",
      role,
    ...
    tokens: { accessToken: `mock_access_${role}`, expiresIn: 900 },
  };
}
...
export const schedules: Schedule[] = [
  {
    ...
    status: "scheduled",
    publishAt: "2026-07-21T04:30:00Z",
    publishAtLocal: "2026-07-21T09:30:00+05:00",
...
export const scraperJobs: ScraperJob[] = [
  {
    ...
    intervalHours: null,
    nextRunAt: null,
    lastRunAt: "2026-07-14T08:04:00Z",
    pagesProcessed: 12,
    failureReason: null,
    attempts: 1,
    createdAt: "2026-07-14T08:00:00Z",
    updatedAt: "2026-07-14T08:04:00Z",
  },
  {
    id: "scrape_2",
    accountId: "tenant_orion",
    createdBy: "user_1",
    jobType: "url",
    target: "https://example.com/ai-workflow-benchmark",
    name: "AI workflow benchmark",
    maxPages: 14,
    status: "running",
    recurring: false,
    intervalHours: null,
  ...
  },
  {
    ...
    createdBy: "user_1",
    jobType: "serp",
    target: "creator economy signals",
...
export const scrapedDocuments: ScrapedDocument[] = [
  {
    id: "doc_1",
    jobId: "scrape_1",
    accountId: "tenant_orion",
    ...
    title: "Acme Pricing",
    text: "Acme pricing starts at $49/month for the Starter plan and scales to custom Enterprise pricing.",
    data: { statusCode: 200, links: [] },
    fetchedAt: "2026-07-14T08:04:00Z",
  },
];
