...
from credits_service.models import CREDITS_SCHEMA, Base
...
def configure(connection: Connection | None = None) -> None:
    context.configure(
        ...
        version_table_schema=CREDITS_SCHEMA,
...
async def online() -> None:
    ...
    async with engine.connect() as connection:
        await connection.execute(text(f'CREATE SCHEMA IF NOT EXISTS "{CREDITS_SCHEMA}"'))
...
if context.is_offline_mode():
    configure()
    context.execute(f'CREATE SCHEMA IF NOT EXISTS "{CREDITS_SCHEMA}"')
    with context.begin_transaction():
        context.run_migrations()
else:
    asyncio.run(online())
